...
Sci_PositionU FindNextMBoxHeader(Sci_PositionU currentLine, IDocument *pAccess) {

    while(documentStateMap[pAccess].find(currentLine+1) != documentStateMap[pAccess].end() && documentStateMap[pAccess][currentLine+1] != SCE_MBOX_FROM ) {
        currentLine++;
    }
...
void SCI_METHOD LexerMBox::Fold(Sci_PositionU startPos, Sci_Position lengthDoc, int initStyle, IDocument *pAccess) {
    ...
    LexAccessor styler(pAccess);
    Sci_PositionU currentLine = styler.GetLine(startPos);
    Sci_PositionU endLine =  FindNextMBoxHeader(currentLine, pAccess);
    int baseLevel = styler.LevelAt(0) & SC_FOLDLEVELNUMBERMASK;

    while (currentLine < endLine) {
        if (documentStateMap[pAccess][currentLine] == SCE_MBOX_FROM) {
            styler.SetLevel(currentLine, baseLevel|SC_FOLDLEVELHEADERFLAG);
        }
        else {
            if (currentLine == endLine-1 && documentDataMap[pAccess][currentLine] == SCE_MBOX_BLANK_LINE) {
                styler.SetLevel(currentLine, baseLevel);

            }
            else {
                styler.SetLevel(currentLine, baseLevel+1);
            }
        }
